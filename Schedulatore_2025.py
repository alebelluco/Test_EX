...
with tab4:
    ...
    if st.toggle('Rimuovi zanzare'):
        ...
        df_raw = df_raw[df_raw.Servizio != 'ANTILARVALE CADITOIE FERRARA - GIRO 5 - SQUADRA 1']
        df_raw = df_raw[df_raw.Servizio != 'ANTILARVALE CADITOIE FERRARA - GIRO 7 - SQUADRA 1']
        df_raw = df_raw[df_raw.Servizio != 'ANTILARVALE CANALI - GIRO 8']
...
# interventi in standby
    ...
    chiusi['data_AB'] = [datetime.strptime(str(stringa)[0:10],"%d/%m/%Y" ) for stringa in chiusi['Data Esecuzione']]
...
# se il contratto è SPOT, metto di default 2 interventi sul mese, così diventa 15 gg la distanza
    ...
    for i in range(len(output)):
        ...
        if frequenza == 1: # servizi previsti una sola volta nel mese        
            ...
            if output['ultimo_intervento'].astype(str).iloc[i]=='NaT' or output['ultimo_intervento'].astype(str).iloc[i]=='nan': #---------sono in stato di pianificazione libera e un singolo intervento
...
#*** Inserisco qui la condizione 15 giorni per i contratti spot??------------------------------- if spot --> periodo = 15
            ...
            if output['rank'].iloc[i] == 1: # è il primo degli interventi multpli è il 1 di n
                planned.loc[i]=output.iloc[i]
                if output['ultimo_intervento'].astype(str).iloc[i]=='NaT' or output['ultimo_intervento'].astype(str).iloc[i]=='nan':
    ...
    altri_siti = altri_siti.merge(df_raw[['ID','PrezzoEUR']],how='left', left_on='ID',right_on='ID')
    altri_siti['PrezzoEUR']=altri_siti['PrezzoEUR'].fillna(0)
