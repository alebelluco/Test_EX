# To-do
# N per interventi extra, da li +5gg
# Aggiornare Clara allowed
# Introdurre nota orario consentito --- lo aggiungo nelle istruzioni operative?
# aggiornare con interventi scaduti / standby

# correggere il primo posizionamento (in base al contratto [tipo o spot], assume valori diversi)
...
with tab1:
    ...
    with col_a:
        ...
        st.write('Versione aggionata del 23/02/2024 - Modifiche introdotte:')
        st.write('- Corretta frequenza per interventi SPOT')
        st.write('- Introdotti interventi stato N')
        st.write('- Aggiornato database disponnibilità giorni per siti')
        st.write('- Introdotta colonna orari consentiti per siti con limitazioni di orario')
...
# keysito
    ...
    altri_siti = df[['ID','Cliente','Sito','Indirizzo Sito','IstruzioniOperative','orari','Note','Servizio','Periodicita','SitoTerritoriale','Citta','Durata_stimata','Target_range']]
...
with tab3:
    ...
    st.dataframe(altri_siti[altri_siti.columns[1:14]].drop('Note', axis=1), width=2500)
